...
// SysEx data bytes must be 0x00–0x7F.
// Every 7 raw bytes → 8 SysEx bytes:
//   Byte 0 = MSB flags (bit j = MSB of raw byte j+1)
...
/** Request the currently-loaded patch on a synth track (device dumps it back). */
export function buildRequestCurrentPatch(synthTrack = SYNTH_TRACK_1) {
  return [...header(CMD_REQUEST_CURRENT_PATCH, synthTrack), 0xF7]
}

/** Request a single patch from the bank by index (bank always 0 on Circuit Tracks). */
export function buildRequestPatchDump(patchIndex, bank = 0) {
  return [...header(CMD_REQUEST_PATCH_DUMP, bank, patchIndex), 0xF7]
}

/**
 * "Replace Current Patch" — sends patch data to the device immediately and
 * auditions it live.  Does NOT write to a bank slot.
 * rawBytes: Array/Uint8Array of PATCH_DATA_BYTES (220) raw bytes.
 */
...
/**
 * "Write Patch" — writes patch data to a bank slot.
 * rawBytes: 220 raw patch bytes; patchIndex: 0–63.
 */
...
/**
 * Build a "Patch Dump" SysEx message — the same format the device uses when
 * responding to a dump request.  Used when building .syx export files.
 */
...
/**
 * Build a full-bank .syx byte array (64 patch dump messages concatenated).
 * patches: Array[64] of raw-byte arrays (220 bytes each); missing → default.
 * Returns a flat Uint8Array ready to write to disk / download.
 */
...
/**
 * Parse a .syx file buffer and return an array of { patchIndex, bank, rawBytes }.
 * Handles single-patch and 64-patch bank files.
 */
...
function _parseSinglePatchMessage(msg) {
  ...
  if (msg[6] !== CMD_PATCH_DUMP) return null  // only accept patch dump messages
...
// ── Patch parameter parsing ───────────────────────────────────────────────────

/** Decode the 16-byte ASCII name from raw patch bytes. */
...
/** Encode a patch name string back into 16 raw bytes. */
...
/** Extract a structured parameter object from raw patch bytes. */
export function rawBytesToParams(raw) {
  ...
  return {
    ...
    oscA: {
      wave: b[P.OSC_A_WAVE] ?? 2,
      waveInterp: b[P.OSC_A_WAVE_INTERP] ?? 0,
      coarse: b[P.OSC_A_COARSE] ?? 64,
      fine:   b[P.OSC_A_FINE]   ?? 64,
      pw:     b[P.OSC_A_PW]     ?? 64,
      mix:    b[P.OSC_A_MIX]    ?? 100,
      fm:     b[P.OSC_A_FM]     ?? 0,
      ring:   b[P.OSC_A_RING]   ?? 0,
    },
    oscB: {
      wave: b[P.OSC_B_WAVE] ?? 2,
      waveInterp: b[P.OSC_B_WAVE_INTERP] ?? 0,
      coarse: b[P.OSC_B_COARSE] ?? 64,
      fine:   b[P.OSC_B_FINE]   ?? 64,
      pw:     b[P.OSC_B_PW]     ?? 64,
      mix:    b[P.OSC_B_MIX]    ?? 0,
    ...
    filter: {
      cutoff:    b[P.FILTER_CUTOFF]    ?? 100,
      resonance: b[P.FILTER_RESONANCE] ?? 0,
      drive:     b[P.FILTER_DRIVE]     ?? 0,
      type:      b[P.FILTER_TYPE]      ?? 1,
      envAmt:    b[P.FILTER_ENV_AMT]   ?? 0,
      velocity:  b[P.FILTER_VELOCITY]  ?? 0,
    },
    ...
    lfo1: {
      wave:  b[P.LFO1_WAVE]  ?? 0,
      rate:  b[P.LFO1_RATE]  ?? 64,
      depth: b[P.LFO1_DEPTH] ?? 0,
      delay: b[P.LFO1_DELAY] ?? 0,
      fade:  b[P.LFO1_FADE]  ?? 0,
      sync:  b[P.LFO1_SYNC]  ?? 0,
    },
    lfo2: {
      wave:  b[P.LFO2_WAVE]  ?? 0,
      rate:  b[P.LFO2_RATE]  ?? 64,
      depth: b[P.LFO2_DEPTH] ?? 0,
      delay: b[P.LFO2_DELAY] ?? 0,
      fade:  b[P.LFO2_FADE]  ?? 0,
      sync:  b[P.LFO2_SYNC]  ?? 0,
    },
    modEnvs: [
    ...
    chorus: {
      enable:   b[P.CHORUS_ENABLE]   ?? 0,
      rate:     b[P.CHORUS_RATE]     ?? 64,
      depth:    b[P.CHORUS_DEPTH]    ?? 64,
      feedback: b[P.CHORUS_FEEDBACK] ?? 0,
...
/** Write structured params back into a raw byte array (returns new array). */
export function paramsToBytesPartial(params, existingRaw) {
  ...
  if (params.modEnvs) {
    const offs = [[P.MENV1_A,P.MENV1_D,P.MENV1_S,P.MENV1_R],[P.MENV2_A,P.MENV2_D,P.MENV2_S,P.MENV2_R],[P.MENV3_A,P.MENV3_D,P.MENV3_S,P.MENV3_R]]
